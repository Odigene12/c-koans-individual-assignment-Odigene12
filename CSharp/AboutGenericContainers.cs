...
namespace DotNetKoans.CSharp
{
	public class AboutGenericContainers : Koan
	{
		...
		public void ConvertingToFixedSizeArray()
		{
			...
			Assert.Equal(numbers, list.ToArray());
		}
		[Koan(13)]
		public void InsertingInTheMiddle()
		{
			List<int> list = new List<int> { 1, 2, 3 };
			list.Insert(1, 6);
            int[] newarray = new int[] { 1, 6, 2, 3 };
			Assert.Equal(newarray, list.ToArray());
		}
		[Koan(14)]
		...
		public void StackOrder()
		{
			...
			Assert.Equal(newarray, stack.ToArray());
		}
		[Koan(18)]
		...
		public void Shifting()
		{
			...
			list.AddFirst("Say");
            var newarray = new[] { "Say", "Hello", "World" };
			Assert.Equal(newarray, list.ToArray());

			list.RemoveLast();
            var RemoveLastInArray = new[] { "Say", "Hello" };
            Assert.Equal(RemoveLastInArray, list.ToArray());

			list.RemoveFirst();
            var RemoveFirstInArray = new[] {"Hello" };
            Assert.Equal(RemoveFirstInArray, list.ToArray());

			list.AddAfter(list.Find("Hello"), "World");
            var AddAfterArray = new[] { "Hello", "World" };
            Assert.Equal(AddAfterArray, list.ToArray());
		}
		[Koan(21)]
